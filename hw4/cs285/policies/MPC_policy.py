...
class MPCPolicy(BasePolicy):
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        for model in self.dyn_models: 
            pred  = self.calculate_sum_of_rewards(obs, candidate_action_sequences, model)
            pred_sum_of_rewards.append(pred)
        
        mean_sum_of_rewards = np.array(pred_sum_of_rewards).mean(axis=0)
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        """
        batch_size, horizon, ac_dim = candidate_action_sequences.shape
        
        rewards_list = []
        predicted_obs = np.broadcast_to(obs, (batch_size, obs.shape[0]))
        for h in range(horizon):
            actions = candidate_action_sequences[:,h,:].squeeze()
        ...
        sum_of_rewards = np.array(rewards_list).sum(axis=0)
        # TODO (Q2)
